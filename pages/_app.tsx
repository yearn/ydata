    ...
    <Header>
      <div className={'flex-row-center w-full justify-between pr-4'}>
        ...
        <div className={'hidden flex-row items-center space-x-6 md:flex'}>
          <div className={'cursor-pointer'} onClick={(): void => set_shouldDisplayPrice(!shouldDisplayPrice)}>
            {shouldDisplayPrice ? (
              <p className={'text-primary-500'}>{`YFI $ ${YFIBalance.normalizedPrice}`}</p>
            ) : (
              <p className={'text-primary-500'}>{`Balance: ${YFIBalance.normalized} YFI`}</p>
            )}
          </div>
        </div>
      </div>
